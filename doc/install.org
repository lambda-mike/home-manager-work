...
=sudo dd bs=4M conv=fsync oflag=direct status=progress if=<path-to.img> of=/dev/sdX=
...
If user is declared in configuration:
...
** Home Manager NixOS module flake template

# https://home-manager.dev/manual/24.11/index.xhtml#sec-flakes-nixos-module
